...
#                st.plotly_chart(fig)
            if pollutants:
                df = pd.DataFrame(pollutants.items(), columns=["Pollutant", "Value"])
                df = df.sort_values(by="Value", ascending=False)
                chart = alt.Chart(df).mark_bar().encode(x=alt.X("Pollutant:N", sort=None, title="Pollutant"),
                y=alt.Y("Value:Q", title="Value (μg/m³)"), tooltip=["Pollutant", "Value"]).properties(title="Pollutant Levels (μg/m³)")
